...
class Manage extends CI_Controller
{
	...
	public function save()
	{
		...
		if ($this->user->get_by_username($username)) {
			$this->session->set_flashdata(array('status' => "danger", 'message' => "Username telah digunakan"));
			redirect($_SERVER['HTTP_REFERER']);
		}

		if ($password !== $repassword) {
			$this->session->set_flashdata(array('status' => "danger", 'message' => "Perulangan Password tidak benar"));
			redirect($_SERVER['HTTP_REFERER']);
		}
