							...
							<form action="<?= base_url()?>dtks/manage/import" method="POST" enctype="multipart/form-data" id="importForm">
								<div class="alert alert-info">
									...
									<ul class="mb-0">
										...
										<li>File CSV (.csv): Kolom pertama harus berisi NIK (jika ada header, NIK di baris ke-2 dst)</li>
										<li>File SQL (.sql): INSERT statement untuk tabel tbl_status_dtks</li>
								...
								<div class="form-group">
									...
									<input type="file" class="form-control-file" name="import_file" id="import_file" accept=".xlsx,.xls,.csv,.sql" required>
									<small class="form-text text-muted">Format yang didukung: Excel (.xlsx, .xls), CSV (.csv), SQL (.sql)</small>
								</div>
...
<div class="row mt-3">
	<div class="col-sm-12">
		<div class="iq-card">
			<div class="iq-card-body">
				<h5><i class="ri-download-line"></i> Download Template</h5>
				<p class="text-muted">Download template untuk memudahkan import data</p>
				<a href="<?= base_url()?>dtks/manage/download_template/excel" class="btn btn-outline-success btn-sm mr-2">
					<i class="ri-file-excel-line"></i> Template Excel
				</a>
				<a href="<?= base_url()?>dtks/manage/download_template/csv" class="btn btn-outline-info btn-sm">
					<i class="ri-file-text-line"></i> Template CSV
				</a>
			</div>
		</div>
	</div>
