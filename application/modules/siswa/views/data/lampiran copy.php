	...
	<div class="row mb-3">
		<div class="col-md-12">
			<div class="alert alert-info" role="alert">
				<i class="ri-information-line mr-2"></i> Silahkan download pandua surat keterangan tidak mampu (SKTM)
			</div>
		</div>
	</div>
	<div class="row mb-3">
		<div class="col-md-12">
			<!-- Upload SKTM -->
			 <form action="<?= base_url() ?>siswa/upload_sktm" method="post">
				<input type="hidden" name="id_siswa" value="<?= $get->id_siswa ?>">
				<input type="file" name="file_sktm" class="form-control-file" accept=".pdf,.jpg,.jpeg,.png">
				<button style="width:100%;" type="submit" class="btn btn-primary mt-2">Upload SKTM</button>
			</form>
		</div>
	</div>
	<!-- Privew Upload SKTM dan Download -->
	<div class="row mb-3">
		<div class="col-md-12">
			<?php if ($get->sktm) : ?>
				<a href="<?= base_url() ?>uploads/siswa/<?= $get->sktm ?>" target="_blank" class="btn btn-sm btn-primary"><i class="fa fa-download" aria-hidden="true"></i> Download SKTM</a>
			<?php else : ?>
				<p class="text-danger">SKTM belum diupload.</p>
			<?php endif; ?>
		</div>
	</div>
	<div class="row">
