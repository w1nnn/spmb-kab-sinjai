				...
				<table class="table table-bordered table-hover">
					...
					<tr>
						...
						<td>Foto ukuran 3x4 , file jpg, Max 500 kb<br>
							<input type="file" style="display: inline;width: auto" class="form-control-file" name="foto" <?= ($get->foto == "") ? "required" : ""; ?> accept="image/jpeg,image/x-png" onchange="previewImage(this)">
						</td>
						<td>
...
<script>
	...
	function previewImage(input) {
		...
		// Jika file ada dan file tersebut adalah gambar
		if (file && file.type.startsWith('image/')) {
			var reader = new FileReader();
			reader.onload = function(e) {
				preview.src = e.target.result; // Set src gambar untuk preview
				preview.style.display = 'block'; // Menampilkan gambar
			};
			reader.readAsDataURL(file); // Membaca file sebagai URL gambar
		} else {
			preview.style.display = 'none'; // Menyembunyikan gambar jika bukan gambar
		}
	}
	...
	function checkFileSize(input) {
		...
		if (file.size > maxSize) {
			...
			input.value = ""; // Menghapus file yang diupload jika terlalu besar
			document.getElementById('fotoPreview').style.display = 'none'; // Sembunyikan preview
		}
	}

	// Mengaitkan fungsi ke input file
	document.querySelectorAll('input[type="file"]').forEach(function(input) {
		input.addEventListener('change', function() {
			checkFileSize(input); // Validasi setiap kali file dipilih
			previewImage(input); // Menampilkan preview gambar
		});
	});
</script>
...
<!-- <script>
	...
	function checkFileSize(input) {
		...
		var maxSize = 500 * 1024; // 500KB dalam byte
		if (file.size > maxSize) {
			alert("File tidak boleh melebihi 500KB");
			input.value = ""; // Menghapus file yang diupload jika terlalu besar
		}
	}
