	...
	public function save()
{
	...
	if ($page == "jalur") {
		...
		// Simpan jenis lampiran dari modal
		$jenis_lampiran = $this->input->post('jenis_lampiran');
		if ($jenis_lampiran) {
			$jenis_data = array('jenis_lampiran' => $jenis_lampiran);
			$this->siswa->update(array('id_siswa' => $id), $jenis_data);
		}
		...
		// Upload SKTM (Surat Keterangan Tidak Mampu)
		if ($this->upload->do_upload("sktm")) {
			$sktm = $this->siswa->get_by_id($id)->sktm;
			if ($sktm && is_file(FCPATH . '/uploads/siswa/' . $sktm)) {
				unlink(FCPATH . '/uploads/siswa/' . $sktm);
			}
			$file = $this->upload->data();
			$filename = $file['file_name'];
			$sktm = array('sktm' => $filename);
			$this->siswa->update(array('id_siswa' => $id), $sktm);
		} 
